  ...
  protected Boolean detcetEligibleIfStmt(soot.jimple.IfStmt unitOfTheBodyMethod , boolean localfromparamconsidered){
	  ...
	  if (localfromparamconsidered) {
		...
		List<Unit>	dedofAt =simpleLocalDefs.getDefsOfAt((Local)Condtionexpr.getOp1(), unitOfTheBodyMethod);
			...
			//on fait return false pour que seul les condition sur les variable initialise à partr de parametre et sur les parametre  soit considerer 
			return false;
		  ...
		  System.out.println(PathLength+"_____path ___"+theConsideredSucessor);
			if (theConsideredSucessor instanceof ThrowStmt ) {
				...
				mustExitWhileLoop=true;
				 System.out.println(">>>>>--------+on a trouver exitpoint suit le if stmt mais  cet exit stmt n'est pas immediatemet à la suit du if  donc on ne dois pas le considérer------->>>>   "+theConsideredSucessor +" suit " + ifstmt);
				//Todo faire la separation entre deux cas celui ou la PathLength< 2 donc posibilité de patern detecter mais pas à 100%
